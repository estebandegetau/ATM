...
ATMS<- s52 %>% filter(codigo_act=="522110") %>%
  filter(          str_detect(string = (nom_estab),
                   pattern = regex("atm|cajero|AUTOM[ÁA]TICO", ignore_case = TRUE))&
                    !str_detect(string = (nom_estab),
                    pattern = regex("BANCO", ignore_case = TRUE))

                       )

Suc<- s52 %>% filter(codigo_act=="522110") %>%
  filter(!str_detect(string = (nom_estab),
                    pattern = regex("atm|cajero|AUTOM[ÁA]TICO", ignore_case = TRUE))&
           str_detect(string = (nom_estab),
                       pattern = regex("BANCO", ignore_case = TRUE)))
...
bbdI22<- bbdI22 %>% filter(str_detect(string = tolower(nom_estab),
...
bbdI21i<- bbdI21 %>% filter(codigo_act=="522210") %>%
  filter(str_detect(string = tolower(nom_estab),
                    pattern = regex("banco del bienestar|bansefi", ignore_case = TRUE)))

bbdI21ii<- bbdI21 %>% filter(codigo_act=="522110") %>%
  filter(!str_detect(string = (nom_estab),
                     pattern = regex("atm|cajero|AUTOM[ÁA]TICO", ignore_case = TRUE))&
           str_detect(string = (nom_estab),
                      pattern = regex("BANCO", ignore_case = TRUE)))
...
bbdI20i<- bbdI20 %>% filter(codigo_act=="522210") %>%
  filter(str_detect(string = tolower(nom_estab),
    pattern = regex("banco del bienestar|bansefi", ignore_case = TRUE)))

bbdI20ii<- bbdI20 %>% filter(codigo_act=="522110") %>%
  filter(!str_detect(string = (nom_estab),
                     pattern = regex("atm|cajero|AUTOM[ÁA]TICO", ignore_case = TRUE))&
           str_detect(string = (nom_estab),
                      pattern = regex("BANCO", ignore_case = TRUE)))
...
M1<- ggplot(data = mexico_map %>% filter(CVE_ENT=="09"|CVE_ENT=="15")) +
  geom_sf(fill = "#699672", color = "white") + # Draw the Mexico shapefile as the base layer
  geom_sf(data = centro %>%
            filter(str_detect(string = tolower(Description),
                              pattern = regex("CIUDAD DE MEXICO|MEXICO", ignore_case = TRUE))),
          color = "#C83942") + # Add Centro layer points
  geom_sf(data = Suc %>%
...
M2<- ggplot(data = mexico_map %>% filter(CVE_ENT=="09"|CVE_ENT=="15")) +
  geom_sf(fill = "#699672", color = "white") + # Draw the Mexico shapefile as the base layer
  geom_sf(data = bbdI21i %>%
            filter(cve_ent=="09"|cve_ent=="15"),
          color = "#C83942") +
  geom_sf(data = bbdI21ii %>%
            filter(cve_ent=="09"|cve_ent=="15"),
          color = "#091736") +
